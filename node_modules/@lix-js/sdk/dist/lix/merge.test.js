...
test("it should copy changes from the sourceLix into the targetLix that do not exist in targetLix yet", async () => {
    ...
    const mockEdges = [{ parent_id: "2", child_id: "3" }];
    const mockPlugin = {
        key: "mock-plugin",
    ...
    await sourceLix.db
        .insertInto("snapshot")
        .values([mockSnapshots[0], mockSnapshots[1], mockSnapshots[2]].map((s) => ({
        content: s.content,
    })))
        .execute();
    await sourceLix.db
    ...
    const snapshots = await targetLix.db
        ...
        .select("id")
        .execute();
    expect(snapshots.map((c) => c.id)).toStrictEqual(expect.arrayContaining([
        "no-content",
        mockSnapshots[0]?.id,
        mockSnapshots[1]?.id,
        mockSnapshots[2]?.id,
    ...
    const edges = await targetLix.db
        ...
        .selectAll()
        .execute();
    expect(edges).toEqual(expect.arrayContaining([expect.objectContaining(mockEdges[0])]));
...
test.todo("it should save change conflicts", async () => {
    ...
    await sourceLix.db
        .insertInto("snapshot")
        .values([mockSnapshots[0], mockSnapshots[2]].map((s) => {
        return { content: s.content };
    }))
        .execute();
    await targetLix.db
...
test("diffing should not be invoked to prevent the generation of duplicate changes", async () => {
    ...
    const snapshotsOnlyInTargetLix = [];
    const changesOnlyInTargetLix = [];
    const snapshotsOnlyInSourceLix = [
        mockJsonSnapshot({ id: "mock-id", color: "blue" }),
    ];
    const changesOnlyInSourceLix = [
        {
            id: "2",
            schema_key: "mock",
            entity_id: "value1",
    ...
    const mockPluginInSourceLix = {
        key: "mock-plugin",
        detectChanges: vi.fn().mockResolvedValue([]),
        detectConflicts: vi.fn().mockResolvedValue([]),
        applyChanges: vi.fn().mockResolvedValue({ fileData: new Uint8Array() }),
    };
    const mockPluginInTargetLix = {
        key: "mock-plugin",
        detectChangesGlob: "*",
        detectChanges: vi.fn().mockResolvedValue([]),
        detectConflicts: vi.fn().mockResolvedValue([]),
    ...
    await sourceLix.db
        .insertInto("snapshot")
        .values([...commonSnapshots, ...snapshotsOnlyInSourceLix].map((s) => {
        return { content: s.content };
    }))
        .execute();
    await sourceLix.db
    ...
    await targetLix.db
        .insertInto("snapshot")
        .values([...commonSnapshots, ...snapshotsOnlyInTargetLix].map((s) => {
        return { content: s.content };
    }))
        .execute();
    await targetLix.db
...
test.todo("it should apply changes that are not conflicting", async () => {
    ...
    const edges = [{ parent_id: "1", child_id: "2" }];
    const mockPlugin = {
        key: "mock-plugin",
        applyChanges: async ({ lix, changes }) => {
            ...
            const fileData = new TextEncoder().encode(JSON.stringify(snapshot.content ?? {}));
            return { fileData };
        },
    };
    const sourceLix = await openLixInMemory({
    ...
    await sourceLix.db
        .insertInto("snapshot")
        .values([mockSnapshots[0], mockSnapshots[1]].map((s) => ({
        content: s.content,
    })))
        .execute();
    await sourceLix.db
    ...
    expect(file.data).toEqual(new TextEncoder().encode(JSON.stringify(mockSnapshots[1].content)));
});
test.todo("subsequent merges should not lead to duplicate changes and/or conflicts", async () => {
    const commonSnapshots = [mockJsonSnapshot({ id: "mock-id", color: "red" })];
    const commonChanges = [
        {
            id: "1",
...
test("it should naively copy changes from the sourceLix into the targetLix that do not exist in targetLix yet", async () => {
    ...
    await sourceLix.db
        .insertInto("snapshot")
        .values(snapshotsOnlyInSourceLix.map((s) => {
        return { content: s.content };
    }))
        .execute();
    await sourceLix.db
...
test("it should copy discussion and related comments and mappings", async () => {
    ...
    expect(commentsLix2AfterSecondMerge.length).toBe(commentsLix1OnSecondMerge.length + 1);
    await merge({ sourceLix: lix1, targetLix: lix2 });
...
test.skip("it should copy change sets and merge memberships", async () => {
    ...
    expect(changeSet1Items.map((item) => item.change_id)).toEqual(expect.arrayContaining([mockChanges[0]?.id, mockChanges[1]?.id]));
    // expect the second change set to contain only the second change
    expect(changeSet2Items.map((item) => item.change_id)).toEqual(expect.arrayContaining([mockChanges[1]?.id]));
});
