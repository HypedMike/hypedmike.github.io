...
export const route = async (context) => {
    ...
    try {
        ...
        await open.lix.db.transaction().execute(async (trx) => {
            ...
            for (const [versionId, versionChanges] of Object.entries(incomingVersionChanges)) {
                const incomingChanges = await Promise.all(versionChanges.map(async (change) => {
                    ...
                    if (foundChange === undefined) {
                        ...
                        if (fallback) {
                            ...
                            throw new Error("Change not found. Expected to find a change for the version change in the incoming changes.");
                        }
                    }
                    return foundChange;
                }));
                ...
                await Promise.all(incomingChanges.map(async (change) => {
...
//# sourceMappingURL=push-v1.js.map