...
test("it should push data successfully", async () => {
    ...
    const response = await lsaHandler(new Request("http://localhost:3000/lsp/push-v1", {
        method: "POST",
        body: JSON.stringify({
            lix_id: id,
            vector_clock: [
...
test("it should return 500 for an invalid Lix file", async () => {
    ...
    const response = await lsa(new Request("http://localhost:3000/lsp/push-v1", {
        method: "POST",
        body: JSON.stringify({
            lix_id: "invalid-lix",
            vector_clock: [],
...
test.skip("it should detect conflicts", async () => {
    ...
    const conflictSnapshots = await lixOnClient.db
        ...
        .innerJoin("change_conflict", "change_set_element.change_set_id", "change_conflict.change_set_id")
        .innerJoin("version_change_conflict", "change_conflict.id", "version_change_conflict.change_conflict_id")
...
//# sourceMappingURL=push-v1.test.js.map