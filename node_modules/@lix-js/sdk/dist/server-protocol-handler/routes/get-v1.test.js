...
test("it should fetch the lix file from the server", async () => {
    ...
    // Fetch the lix file
    const response = await lspHandler(new Request("http://localhost:3000/lsp/get-v1", {
        method: "POST",
        headers: {
...
test("lix_sync is set to true", async () => {
    ...
    // Store the lix file
    const response0 = await lspHandler(new Request("http://localhost:3000/lsp/new-v1", {
        method: "POST",
        body: await toBlob({ lix }),
    }));
    const id = (await response0.json()).id;
    const response1 = await lspHandler(new Request("http://localhost:3000/lsp/get-v1", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
...
//# sourceMappingURL=get-v1.test.js.map