...
test("it should pull rows successfully", async () => {
    ...
    const response = await lsaHandler(new Request("http://localhost:3000/lsp/pull-v1", {
        method: "POST",
        body: JSON.stringify({
            lix_id: id.value,
    ...
    const changeTable = Object.entries(responseJson.data).find((table) => table[0] === "change");
...
test("it should specifically be able to handle snapshots which use json binary and should not transfer the id", async () => {
    ...
    const response = await lsa(new Request("http://localhost:3000/lsp/pull-v1", {
        method: "POST",
        body: JSON.stringify({
            lix_id: id,
            vector_clock: [],
    ...
    const responseJson = (await response.json());
    expect(responseJson.data).toBeDefined();
    const snapshots = Object.entries(responseJson.data).find((table) => table[0] === "snapshot");
...
//# sourceMappingURL=pull-v1.test.js.map