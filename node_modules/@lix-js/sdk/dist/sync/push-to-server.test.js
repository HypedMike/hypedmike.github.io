...
test("push rows of multiple tables to server successfully", async () => {
    ...
    await lspHandler(new Request("http://localhost:3000/lsp/new-v1", {
        method: "POST",
...
test("push-pull-push with two clients", async () => {
    ...
    const client2KeyValueAfterPull = await client2.db
        ...
        .selectAll()
        .execute();
    expect(client2AccountAfterPull).toEqual(expect.arrayContaining([
        { id: "account0", name: "account from client 1" },
    ]));
    expect(client2KeyValueAfterPull).toEqual(expect.arrayContaining([
        expect.objectContaining({
            key: "mock-key",
            value: "mock-value from client 1",
        }),
    ...
    const accountsChangesOnServer = await openOnServer.lix.db
        ...
        .selectAll()
        .execute();
    expect(accountsChangesOnServer.map((c) => c.content)).toEqual(expect.arrayContaining([
        { id: "account0", name: "account from client 1" },
    ...
    const keyValueChangesOnServer = await openOnServer.lix.db
        ...
        .selectAll()
        .execute();
    expect(keyValueChangesOnServer.map((c) => c.content)).toEqual(expect.arrayContaining([
        expect.objectContaining({
            key: "mock-key",
            value: "mock-value from client 1",
        }),
...
test.todo("it should handle binary values", async () => {
    ...
    global.fetch = vi.fn((request) => lspHandler(request));
    // initialize the lix on the server
    await lspHandler(new Request("http://localhost:3000/lsp/new", {
        method: "POST",
        body: await toBlob({ lix }),
    }));
...
//# sourceMappingURL=push-to-server.test.js.map