...
import { createLspInMemoryEnvironment, createServerProtocolHandler, } from "../server-protocol-handler/index.js";
...
test("versions should be synced", async () => {
    ...
    await lspHandler(new Request("http://mock.com/lsp/new-v1", {
        method: "POST",
    ...
    // start syncing
    await Promise.all([lix0, lix1].map((lix) => lix.db
    ...
    const [lix0VersionChanges, lix1VersionChanges] = await Promise.all([lix0, lix1].map((lix) => lix.db
...
test("doesnt sync if lix_sync is not true", async () => {
    ...
    const keyValueChangesOnServerAfterSync = await server.lix.db
        ...
        .selectAll()
        .execute();
    expect(keyValueChangesOnServerAfterSync).toEqual(expect.arrayContaining([
        expect.objectContaining({
...
//# sourceMappingURL=sync-process.test.js.map