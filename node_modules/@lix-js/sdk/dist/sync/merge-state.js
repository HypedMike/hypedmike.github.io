import { tableIdColumns, tablesByDepencies, } from "../database/mutation-log/database-schema.js";
export async function mergeTheirState(args) {
    const executeInTransaction = async (trx) => {
        ...
        const moreRecentRowUpdatesUnknownBySource = unrecognizedSesionTicks.length === 0
            ? []
            : await trx
                .selectFrom("mutation_log")
                .select(({ fn }) => {
                ...
                .where((eb) => {
                return eb.or(unrecognizedSesionTicks.map(({ session, time }) => {
        ...
        const rowsIUpdatedLast = moreRecentRowUpdatesUnknownBySource.reduce((acc, { table_name, row_id, last_updated_wall_time }) => {
            if (!acc[table_name]) {
                acc[table_name] = {};
            }
        ...
        for (const tableName of tablesByDepencies) {
            ...
            for (const row of tableRows) {
                // use a compound id? -
                if (rowsIUpdatedLast[tableName]?.[rowIdToString(tableName, row)] ===
                    undefined) {
                    ...
                    console.log("Row skiped - " + tableName + " - " + rowIdToString(tableName, row));
                }
            }
        }
