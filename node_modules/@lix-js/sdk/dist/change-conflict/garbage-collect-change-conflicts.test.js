...
test.skip("should garbage collect conflicts that contain one or more changes that no version change pointer references (anymore)", async () => {
    ...
    const remainingConflictElements = await lix.db
        ...
        .innerJoin("change_conflict", "change_conflict.change_set_id", "change_set_element.change_set_id")
        .selectAll()
        .execute();
    ...
    expect(remainingVersionChangeConflictPointers[0]?.change_conflict_id).toBe(mockConflict0.id);
});
...
//# sourceMappingURL=garbage-collect-change-conflicts.test.js.map