...
export async function createChangeConflict(args) {
    const executeInTransaction = async (trx) => {
        ...
        const existingConflict = await trx
            ...
            .innerJoin("version_change_conflict", "version_change_conflict.change_conflict_id", "change_conflict.id")
            ...
            .where((eb) => eb.exists(trx
            .selectFrom("change_set_element")
            .innerJoin("change_conflict", (join) => join.onRef("change_conflict.change_set_id", "=", "change_set_element.change_set_id"))
            .where("change_set_element.change_id", "in", Array.from(args.conflictingChangeIds))
            .groupBy("change_set_element.change_set_id")
            .having(trx.fn.count("change_set_element.change_id"), "=", args.conflictingChangeIds.size)
        ...
        await trx
            ...
            .values(Array.from(args.conflictingChangeIds).map((id) => ({
            change_id: id,
...
//# sourceMappingURL=create-change-conflict.js.map