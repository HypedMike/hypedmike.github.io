...
test("it should resolve a conflict and apply the changes", async () => {
    ...
    const mockPlugin = {
        ...
        applyChanges: vi.fn().mockResolvedValue({
            fileData: new TextEncoder().encode(JSON.stringify(mockSnapshots[0]?.content)),
        }),
    };
    ...
    const resolvedConflict = await lix.db
        ...
        .innerJoin("change_conflict_resolution", "change_conflict_resolution.change_conflict_id", "change_conflict.id")
...
//# sourceMappingURL=resolve-conflict-by-selecting.test.js.map