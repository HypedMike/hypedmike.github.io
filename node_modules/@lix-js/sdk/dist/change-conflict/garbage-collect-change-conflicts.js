...
export async function garbageCollectChangeConflicts(args) {
    const executeInTransaction = async (trx) => {
        ...
        const conflictsWithVersionPointer = await trx
            ...
            .leftJoin("version_change_conflict", "change_conflict.id", "version_change_conflict.change_conflict_id")
        ...
        const outdatedConflicts = await trx
            .selectFrom("change_conflict")
            .leftJoin("change_set_element as conflict_elements", "change_conflict.change_set_id", "conflict_elements.change_set_id")
            .leftJoin("change_set_element as version_elements", "conflict_elements.change_id", "version_elements.change_id")
            ...
            .having(sql `count(version_elements.change_id)`, 
        ...
        if (conflictsToDelete.length > 0) {
            ...
            await trx
                ...
                .where("change_set_element.change_set_id", "in", conflictsToDelete.map((conflict) => conflict.change_set_id))
                .execute();
            await trx
...
//# sourceMappingURL=garbage-collect-change-conflicts.js.map