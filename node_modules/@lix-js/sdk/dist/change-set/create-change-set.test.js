...
test("creating a change set should succeed", async () => {
    ...
    const changeSetMembers = await lix.db
        ...
        .execute();
    expect(changeSetMembers.map((member) => member.change_id)).toEqual(expect.arrayContaining([mockChanges[0]?.id, mockChanges[1]?.id]));
});
...
test("creating a change set with labels should associate the labels with the change set", async () => {
    ...
    expect(changeSetLabels.map((label) => label.name)).toEqual(expect.arrayContaining(["test-label", "checkpoint"]));
});
//# sourceMappingURL=create-change-set.test.js.map