...
test("change graph edges can't reference themselves", async () => {
    ...
    await expect(db
        ...
        .returningAll()
        .execute()).rejects.toThrowErrorMatchingInlineSnapshot(`[SQLite3Error: SQLITE_CONSTRAINT_CHECK: sqlite3 result code 275: CHECK constraint failed: parent_id != child_id]`);
});
test("change set items must be unique", async () => {
    ...
    expect(db
        ...
        .returningAll()
        .execute()).rejects.toThrowErrorMatchingInlineSnapshot(`[SQLite3Error: SQLITE_CONSTRAINT_PRIMARYKEY: sqlite3 result code 1555: UNIQUE constraint failed: change_set_element.change_set_id, change_set_element.change_id]`);
});
...
test("invalid file paths should be rejected", async () => {
    ...
    await expect(db
        ...
        .returningAll()
        .execute()).rejects.toThrowError("File path must start with a slash");
});
test("vector clock functions", async () => {
    ...
    const db = initDb({ sqlite });
    const vectorClockTick1 = await sql `select lix_session() as session, lix_session_clock_tick() as time`.execute(db);
    const vectorClockTick2 = await sql `select lix_session() as session, lix_session_clock_tick() as time`.execute(db);
    expect(vectorClockTick1.rows[0]["session"]).toEqual(vectorClockTick2.rows[0]["session"]);
    expect(vectorClockTick1.rows[0]["time"]).toBeLessThan(vectorClockTick2.rows[0]["time"]);
});
...
test("version_change must have a unique entity_id, file_id, version_id, and schema_id", async () => {
    ...
    await expect(lix.db
        .insertInto("version_change")
        .values({ ...versionChange, version_id: version0.id })
        .execute()).rejects.toThrowErrorMatchingInlineSnapshot(`[SQLite3Error: SQLITE_CONSTRAINT_UNIQUE: sqlite3 result code 2067: UNIQUE constraint failed: version_change.version_id, version_change.entity_id, version_change.schema_key, version_change.file_id]`);
...
test("versions have a unique and default human readable name", async () => {
    ...
    await expect(createVersion({ lix, name: version0.name }), "version.name is unique").rejects.toThrow();
});
//# sourceMappingURL=init-db.test.js.map